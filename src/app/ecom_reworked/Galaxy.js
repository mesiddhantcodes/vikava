...
const Galaxy = memo(function Galaxy({
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (idleId.current) {
                cancelIdleCb(idleId.current);
            }
        };
    }, [isVisible]);

    useEffect(() => {
        if (!ctnDom.current || !shouldRender) return;
        
        const ctn = ctnDom.current;
        
        const pixelRatio = Math.min(window.devicePixelRatio * 0.5, 1.0);
        ...
        function resize() {
            ...
            resizeTimeout = setTimeout(() => {
                if (!ctn) return;
                const width = Math.floor(ctn.clientWidth * pixelRatio);
                const height = Math.floor(ctn.clientHeight * pixelRatio);
        ...
        function update(t) {
            ...
            const currentTargetFPS = isMouseOver.current ? 48 : 24;
            const currentFrameTime = 1000 / currentTargetFPS;
            
            if (deltaTime < currentFrameTime) return;
            
            lastTime = t - (deltaTime % currentFrameTime);
            frameCount++;

            if (!isMouseOver.current && frameCount % 2 === 0) return;
